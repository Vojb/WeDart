...
function applyHiddenDartReplay(
  players: HiddenCricketPlayer[],
  playerIndex: number,
  dart: HiddenCricketDart,
  gameType: HiddenCricketGameState["gameType"]
): void {
  if (dart.targetNumber === "Miss") {
    const p = { ...players[playerIndex] };
    p.dartsThrown += 1;
    p.currentDartIndex += 1;
...
function replayHiddenLegRounds(
  legRounds: HiddenCricketRound[],
  playerTemplates: { id: number; name: string }[],
  gameType: HiddenCricketGameState["gameType"],
  hiddenNumbers: (number | string)[]
): HiddenCricketPlayer[] {
  const players: HiddenCricketPlayer[] = playerTemplates.map((p) => ({
    id: p.id,
    name: p.name,
    totalScore: 0,
    dartsThrown: 0,
  ...
  for (const round of legRounds) {
    ...
    for (const dart of round.darts) {
      applyHiddenDartReplay(players, playerIndex, dart, gameType);
    }
  }
  return players;
}

function removeLastDartFromHiddenLegRounds(
  legRounds: HiddenCricketRound[]
): { modifiedLegRounds: HiddenCricketRound[]; removedDart: HiddenCricketDart } | null {
      ...
      undoLastHit: () => {
        set((state) => {
          ...
          if (
            ...
            const removed = removeLastDartFromHiddenLegRounds(lastLeg.rounds);
            ...
            const replayedPlayers = replayHiddenLegRounds(
              modifiedLegRounds,
              playerTemplates,
              g.gameType,
              lastLeg.hiddenNumbers
            );
            ...
            let rounds: HiddenCricketRound[];
            let currentRound: HiddenCricketRound;
            ...
            return {
              ...
              currentGame: {
                ...
                currentRound,
                currentPlayerIndex,
                hiddenNumbers: [...lastLeg.hiddenNumbers],
                isGameFinished: false,
              },
            };
          }
